...
mod tests {
    ...
    #[test]
    fn test_mcp_tool_schema_nullable_and_integer() {
        // Build an MCP tool with properties that include:
        // - a nullable string using a union type array: ["string", "null"]
        // - an integer type (emitted by common Pydantic schemas)
        let tool = mcp_types::Tool {
            name: "nullable_and_integer".to_string(),
            input_schema: ToolInputSchema {
                properties: Some(serde_json::json!({
                    "nullable_string": {
                        "type": ["string", "null"]
            ...
            title: None,
            annotations: None,
            description: Some("Test tool with nullable and integer".to_string()),
        };

        let converted =
            mcp_tool_to_openai_tool("test_server/nullable_and_integer".to_string(), tool)
                .expect("conversion should succeed");

        // Expect normalization:
        // - nullable_string collapses to a simple string schema
        // - integer_field maps to number, preserving description
        let mut expected_props = BTreeMap::new();
        ...
        let expected = ResponsesApiTool {
            name: "test_server/nullable_and_integer".to_string(),
            description: "Test tool with nullable and integer".to_string(),
            strict: false,
            parameters: JsonSchema::Object {
